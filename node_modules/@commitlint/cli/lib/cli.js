...
function main(options) {
	return new Promise(function ($return, $error) {
		var raw, flags, fromStdin, range, input, messages, loadOpts, loaded, parserOpts, opts, format, results, report, output;
		raw = options.input;
		flags = normalizeFlags(options.flags);
		fromStdin = checkFromStdin(raw, flags);
		range = pick(flags, 'edit', 'from', 'to');
		return Promise.resolve(fromStdin ? stdin() : read(range, { cwd: flags.cwd })).then(function ($await_2) {
			try {
				input = $await_2;
				...
				return Promise.resolve(load(getSeed(flags), loadOpts)).then(function ($await_3) {
					try {
						loaded = $await_3;
						...
						return Promise.resolve(Promise.all(messages.map(message => lint(message, loaded.rules, opts)))).then(function ($await_4) {
							try {
								results = $await_4;
