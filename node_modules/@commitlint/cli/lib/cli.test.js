...
function writePkg(payload, options) {
	return new Promise(function ($return, $error) {
		...
		return Promise.resolve(sander.readFile(pkgPath)).then(function ($await_79) {
			try {
				pkg = JSON.parse($await_79);
				result = (0, _lodash2.default)(pkg, payload);
				return Promise.resolve(sander.writeFile(pkgPath, JSON.stringify(result, null, '  '))).then(function ($await_80) {
					try {
						return $return();
					} catch ($boundEx) {
						return $error($boundEx);
